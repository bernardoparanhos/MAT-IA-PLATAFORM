  ...
  <>
    <nav className="p-4 space-y-1">
      ...
      {[
        ...
        <button key={item.label} onClick={onClick}
...
function Dashboard() {
  ...
  useEffect(() => {
    async function buscarPerfil() {
      try {
        const res = await fetch(`${API}/auth/aluno/perfil`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        const data = await res.json()
        ...
        <main className="flex-1 p-6 lg:p-10 mt-14 lg:mt-0 overflow-y-auto">
          ...
          <div className="space-y-6">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start"> 
             ...
             {/* Minha Turma */}
            <div className="mt-2">
              <p className="text-slate-500 text-xs uppercase tracking-widest mb-4">👥 Minha Turma</p>
              <div className="bg-[#1e2d3d] border border-white/5 rounded-2xl p-5 lg:p-6">
                {turma ? (
                  <>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-slate-400 text-xs font-light mt-0.5">
                          {colegas.length + 1} {colegas.length + 1 === 1 ? 'aluno' : 'alunos'} matriculados
                        </p>
                      </div>
                      <span className="text-xs text-orange-400 bg-orange-500/10 border border-orange-500/20 px-3 py-1 rounded-full">
                        você está aqui
                      </span>
                    </div>
                    {colegas.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {colegas.map((c, i) => (
                          <span key={i} className="text-slate-400 text-xs bg-white/5 border border-white/10 px-3 py-1 rounded-full font-light">
                            {c.nome.split(' ')[0]}
                          </span>
                        ))}
                      </div>
                    ) : (
                      <p className="text-slate-500 text-sm font-light">Você é o primeiro da turma!</p>
                    )}
                  </>
                ) : (
                  <p className="text-slate-500 text-sm font-light">Nenhuma turma vinculada.</p>
                )}
              </div>
            </div>
