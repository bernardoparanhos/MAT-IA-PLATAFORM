...
function DashboardProfessor() {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] flex" style={{fontFamily: 'Outfit, sans-serif'}}>
      ...
      {/* ─── Sidebar ─────────────────────────────────────────── */}
      <aside className="w-56 bg-[#1e2d3d] flex flex-col">
  ...
  {[
    {
      label: 'Início',
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><path d="M3 12L12 3l9 9"/><path d="M9 21V12h6v9"/></svg>
    },
    {
      label: 'Matérias',
    ...
    <button key={item.label} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-slate-400 hover:bg-white/5 hover:text-white transition-colors text-sm font-light">
  ...
  {[
    {
      label: 'Turmas',
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87m-4-12a4 4 0 010 7.75"/></svg>
    },
    {
      label: 'Métricas',
    ...
    {
      label: 'Notificações',
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9m-4.27 13a2 2 0 01-3.46 0"/></svg>
    },
  ].map(item => (
    <button key={item.label} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-slate-400 hover:bg-white/5 hover:text-white transition-colors text-sm font-light">
      <span className="text-slate-500">{item.icon}</span>
...
<div className="p-4 border-t border-white/10 space-y-1">
  <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-slate-400 hover:bg-white/5 hover:text-white transition-colors text-sm font-light">
    ...
    <span>Sair</span>
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { label: 'Média Geral', valor: temTurma ? '—' : '—' },
            ].map(card => (
              <div key={card.label} className="bg-[#1e2d3d] rounded-2xl p-6 border border-white/5">
                <div className="text-4xl font-semibold text-white mb-1">{card.valor}</div>
                <div className="text-slate-400 text-sm font-light">{card.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          {carregando ? (
            ...
            <div className="bg-[#1e2d3d] border border-white/5 rounded-2xl p-10 text-center">
              ...
              <button
                onClick={abrirModal}
                className="bg-orange-500 hover:bg-orange-600 text-white font-medium px-8 py-2.5 rounded-xl text-sm transition-colors"
              >
            ...
            <div className="space-y-3">
              {turmas.map(turma => (
                <div key={turma.id} className="bg-[#1e2d3d] border border-white/5 rounded-2xl p-6 flex items-center justify-between">
                  <div>
      ...
      {modalAberto && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-[#1e2d3d] border border-white/10 rounded-2xl p-8 w-full max-w-md shadow-2xl">
            ...
            <button
              ...
              className="w-full mt-6 text-slate-500 hover:text-slate-300 text-sm transition-colors font-light"
            >
              Cancelar
            </button>
          </div>
