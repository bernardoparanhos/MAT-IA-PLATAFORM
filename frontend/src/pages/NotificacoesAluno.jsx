...
function NotificacoesAluno() {
  ...
  const { notificacoes, naoLidas, marcarLida, apagarUma, apagarTodas } = useNotificacoesAluno()
  ...
  useEffect(() => {
    if (idDestaque && notificacoes.length > 0) {
      ...
      setTimeout(() => {
        navigate('/notificacoes-aluno', { replace: true })
      }, 2000)
    }
  }, [idDestaque, notificacoes, navigate])
        ...
        <main className="flex-1 p-6 lg:p-10 mt-14 lg:mt-0">
          ...
          {notificacoes.length === 0 ? (
            <div className="bg-[#1e2d3d] border border-white/5 rounded-2xl p-10 text-center">
              ...
              <p className="text-slate-400 text-sm font-light">Você não tem notificações no momento.</p>
            </div>
          ) : (
